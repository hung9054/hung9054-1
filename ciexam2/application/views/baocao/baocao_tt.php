				...
				<div id="timkiem1">
					<?php 
						foreach ($title as  $value) {
							echo "<legend>Báo cáo Các MBA Của Đơn Vị ".$value["TEN_DV"]."</legend>";
						}
						if ($title_tt==NULL)
							echo "<legend>Tình Trạng: Tất Cả</legend>";
						else
							foreach ($title_tt as  $v) {
								echo "<legend>Tình Trạng:".$v["TRANGTHAI"]."</legend>";
							}
					 ?>
					 <div class="re">
				     <div id="menu_tk1">
				     ...
				     <div id="result_tk1">
						<table>
							...
							<?php endforeach ?>	
								<tr>
									<td colspan="10" style="padding:5px;">Tổng Cộng: <?php echo $i-1; ?> MBA</td>
								</tr>							
						</table>
						<div id="xuat_excel">
							<?php
                            	foreach ($title_tt as $k=>$t){$matt = $t["MA_TT"];}
								echo '<a href="'.$url.'baocao/xuatexcel_donvi_tt/'.$madv.'/'.$matt.'">Xuất file excel</a>'
							?>
						</div>
					</div><!--End result_tk1-->				
				</div><!--End timkiem2-->
