 ...
 include_once($_SERVER['DOCUMENT_ROOT']."/sharefile/Configs/data.php");
	  class tonghop extends data{
		...
		public function ktgoidangsd($email){
			  $this->setQuery("select MA_LOAI_TV from chi_tiet_nang_cap  where NGAY_KET_THUC >= curdate() and email_tk = '".$email."' ");
			  return $this->fetchAll();
		  }
		...
		public function dungluonggoidangsd($email,$dl){
			  $this->setQuery("select max(a.DUNG_LUONG_LUU_TRU), b.NGAY_KET_THUC,a.MA_LOAI_TV  from loai_goi a,  chi_tiet_nang_cap b where a.MA_LOAI_TV = b.MA_LOAI_TV and b.NGAY_NANG_CAP <= curdate() and b.NGAY_KET_THUC >=curdate()  and b.email_tk = '".$email."' and  a.DUNG_LUONG_LUU_TRU='".$dl."'");
			  return $this->fetchAll();
		  }
		 public function goidangsdht($email){
			  $this->setQuery("select max(a.DUNG_LUONG_LUU_TRU)  from loai_goi a,  chi_tiet_nang_cap b where a.MA_LOAI_TV = b.MA_LOAI_TV and b.NGAY_NANG_CAP <= curdate() and b.NGAY_KET_THUC >=curdate()  and b.email_tk = '".$email."'");
			  return $this->fetchAll();
		  } 
	   ...
	   public function xoa_bc_vp($mafile){
		...
		public function themthumuc($matm,$mail){
            $this->setQuery("insert into thumuc values (".$matm.",'".$mail."','".$mail."',curdate(),0,0)");
            return $this->executeQuery();
        }
